...
class RewardPools extends Component {
  ...
  render() {
    ...
    return (
      <div className={ classes.root }>
        ...
        <Typography variant={'h5'} className={ classes.disclaimer }>Pools 1-3 open on 9/12 4PM CST. Pool 4 opens one day later.</Typography>
        <Typography variant={'h5'} className={ classes.disclaimer }>There is a frontend UI glitch with the staked yNFTs balance. You can still call all functions from etherscan. Funds are safe.</Typography>
  ...
  renderRewards = () => {
    ...
    return rewardPools.filter((rewardPool) => {
      if(['ynft-eth', 'ynft-dai', 'wnxm'].includes(rewardPool.id) && governanceContractVersion === 1) {
        return true
      } else {
        return false
      }
